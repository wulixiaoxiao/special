...
<section class="content-header">
    <h1>
        专题分类列表
        <small>专题分类列表</small>
        <button type="button" class="btn btn-success btn-sm pull-right" onclick="javascript:window.location.href='{{url('admin/special/category/add')}}'">增加</button>
    </h1>
</section>
                ...
                <div class="box-body table-responsive no-padding">
                    <table class="table table-hover">
                        ...
                        <tr>
                            ...
                            <td>
                                <button type="button" class="btn btn-info btn-xs" onclick="window.location.href='{{url('admin/special/category/edit/'.$value['id'])}}'">编辑</button>
    ...
    <script>
        ...
        function del(id) {
            ...
            if (confirm('删除后不可恢复,确定要删除吗?')) {
                $.post('{{url('admin/special/category/del')}}', {'id' : id}, function(data) {
                    if (data.error == 0) {
                        alert('删除成功');
